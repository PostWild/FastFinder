...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [
            [
                InlineKeyboardButton('🙋 Help', url=f"https://t.me/FastFinderBot?start=help"),
            ]
            ]
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        if message.command[1] != "subscribe":
            try:
                ...
                btn.append([InlineKeyboardButton(" 🔄 Try Again", url=f"https://t.me/+pQ4_XH4_1FBmMjIx?start={message.command[1]}")])
...
async def delete_all_index_confirm(bot, message):
    ...
    await message.answer('♥️ Thank You TeamYourX ♥️')
